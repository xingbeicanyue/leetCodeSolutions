...
class Solution:
    def predictPartyVictory(self, senate: str) -> str:
        # 将行动顺序转为数字，R为正数，D为负数
        # nums表示行动顺序，+n表示连续n个R，-n表示连续n个D
        nums = [1 if c == 'R' else -1 for c in senate]
        while True:
            result = self.oneTurn(nums)
            if result > 0:
                return 'Radiant'
            elif result < 0:
                return 'Dire'
    ...
    def oneTurn(self, nums: list) -> int:
        ...
        """
        newNums = []
        acc, partyAcc = nums[0], nums[0]  # 当前领先阵营及领先数量 | 领先阵营累计数 （两者均正数R领先，负数D领先）
        for i in range(1, len(nums)):
            num = nums[i]
            acc += num
            if partyAcc * num > 0:
                partyAcc += num
            elif acc * partyAcc < 0:  # 领先阵营逆转
                newNums.append(partyAcc)
                partyAcc = acc
        newNums.append(partyAcc)
        # 此处acc表示多余的权利，会禁用newNums中敌对阵营的前置位的权利
        # 更新行动顺序列表
        nums.clear()
        for newNum in newNums:
            if newNum * acc < 0:  # 禁用前置位权利
                if abs(acc) >= abs(newNum):
                    acc += newNum
                else:
                    nums.append(newNum + acc)
                    acc = 0
            else:
                nums.append(newNum)
        return acc
...
if __name__ == '__main__':
    s = Solution()
    r = s.predictPartyVictory('RD')
    print(r)
    r = s.predictPartyVictory('RDD')
    print(r)
