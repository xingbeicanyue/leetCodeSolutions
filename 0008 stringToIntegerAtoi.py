...
class Solution:
    def myAtoi(self, s: str) -> int:
        # 将解析分为读取前导空格和读取数字两个阶段
        # 在读取前导空格时读取到正负号或数字则转为读取数字阶段
        # 在读取数字时读取到非数字 或 任何时候读取到非“空格、正负号、数字”时结束解析
        result, isPositive, ord0 = 0, True, ord('0')
        readingDigit = False  # False:可以读取前导空格或正负号; True:只能接收数字
        for c in s:
            if c == ' ':
                if readingDigit:
                    break
                ...
                readingDigit = True
        return min(max(-2 ** 31, result if isPositive else -result), 2 ** 31 - 1)
